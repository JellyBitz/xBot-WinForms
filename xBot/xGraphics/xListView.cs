...
namespace xGraphics
{
	public class xListView : ListView
	{
		...
		[Category("Behavior")]
		public bool DragDropRemoveFromSource
		{
			get { return m_dragDropRemoveFromSource; }
			set { m_dragDropRemoveFromSource = value; }
		}
		[Category("Appearance")]
		...
		protected override void OnDragOver(DragEventArgs drgevent)
		{
			...
			if (base.Items.Count > 0 && m_allowReorder)
			{
				// get the currently hovered row that the items will be dragged to
				Point clientPoint = base.PointToClient(new Point(drgevent.X, drgevent.Y));
				ListViewItem hoverItem = base.GetItemAt(clientPoint.X, clientPoint.Y);
		...
		private class DragItemData
		{
			...
			public DragItemData(xListView listView)
			{
				m_listView = listView;
		...
		#region OnItem Handlers
		public class DragItemEventArgs : EventArgs
		{
			public ListViewItem Item { get; set; }
			/// <summary>
			/// Cancel the item if is getting added.
			/// </summary>
			public bool Cancel { get; set; }
		}
		[Category("DragDrop")]
		public event EventHandler<DragItemEventArgs> DragItemAdding
		{
			add { _OnDragItemAdding += value; }
			remove { _OnDragItemAdding -= value; }
		}
		private EventHandler<DragItemEventArgs> _OnDragItemAdding;
		protected virtual void OnDragItemAdding(DragItemEventArgs e)
		{
			_OnDragItemAdding.Invoke(this, e);
		}
		[Category("DragDrop")]
		public event EventHandler DragItemsChanged
		{
			add { _OnDragItemsChanged += value; }
			remove { _OnDragItemsChanged -= value; }
		}
		private EventHandler _OnDragItemsChanged;
		protected virtual void OnDragItemsChanged(EventArgs e)
		{
			_OnDragItemsChanged?.Invoke(this, e);
		}
		#endregion
	}
}
